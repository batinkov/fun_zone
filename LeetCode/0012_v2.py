...
class Solution:
    def intToRoman(self, num: int) -> str:
        ...
        for value, roman in MAPPING:
            ...
            result.append(roman * count) # roman * 0 is '', so no guard is needed
...
if __name__ == '__main__':
    tests = (
        ...
        (1994, "MCMXCIV"),
        (10, "X"),
        (2121, "MMCXXI"),               # M x2, C, X x2, I
        (232, "CCXXXII"),               # C x2, X x3, I x2
        (363, "CCCLXIII"),              # C x3, L, X, I x3
        (475, "CDLXXV"),                # CD - the one subtractive form the rest never reach
        (586, "DLXXXVI"),               # D, L, X x3, V, I
        (607, "DCVII"),                 # D, C, V, I x2
        (800, "DCCC"),                  # D, C x3
        (1, "I"),                       # minimum of the range
        (3999, "MMMCMXCIX"),            # maximum of the range
        (3888, "MMMDCCCLXXXVIII"),      # longest numeral in range (15 characters)
    )
    sol = Solution()

    for test in tests:
        result = sol.intToRoman(test[0])
